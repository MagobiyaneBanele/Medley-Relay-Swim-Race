///Banele Magobiyane
/*
// Synchronization in the StadiumGrid class ensures thread safety when multiple threads access or modify GridBlocks. It prevents race conditions
// by managing concurrent access to critical sections, such as the entrance block and target blocks during movements. This prevents multiple
// threads from occupying or moving to the same block simultaneously, ensuring consistent and accurate block states throughout the simulation.
*/
...
public class StadiumGrid {
   ...
   public CountDownLatch readylatch = new CountDownLatch(10);
   ...
   private boolean teams[] = new boolean[10];
	...
	// This allows to set a partcular state of a team (currently in the pool or not)
	public void setTeamStatus(int team,boolean status){
      teams[team] = status;
   }
	// checks if a team is in a particular state 
   public boolean getTeamStatus(int team){
      return teams[team];
   }
